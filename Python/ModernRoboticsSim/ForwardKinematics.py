...
# xOff = np.array([10, 10, 10, 10, 10, 10])
# yOff = np.array([10, 10, 10, 10, 10, 10])
# zOff = np.array([10, 10, 10, 10, 10, 10])

def getEndEffectorData(theta):  # more efficient version of update matrices that only returns end effector position array and rotation matrix
	...
	print(transform0Tool)
	# Extract position and rotation data for tool
...
def updateMatrices(theta):
	...
	transform03 = transform02 @ transform[2]
	transform04 = transform03 @ transform[3]
	transform05 = transform04 @ transform[4]
	transform06 = transform05 @ transform[5]
